      ...
      <nav className="sticky top-0 z-50 border-b border-slate-100 bg-white/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden items-center gap-8 md:flex">
            ...
            <a href="#locations" className="text-sm font-medium text-slate-600 hover:text-emerald-600 transition-colors">Locations</a>
            {currentUser ? (
              <div className="flex items-center gap-4">
                <span className="text-sm font-medium text-emerald-700">Hi, {currentUser.full_name?.split(' ')[0]}</span>
                <Link to={currentUser.role === 'customer' ? '/dashboard' : '/portal'} className="flex items-center gap-2 rounded-full bg-emerald-600 px-5 py-2.5 text-sm font-bold text-white transition-all hover:bg-emerald-700 hover:scale-105 shadow-md">
                  Go to Dashboard
                </Link>
              </div>
            ) : (
              <Link to="/login" className="flex items-center gap-2 rounded-full bg-slate-900 px-5 py-2.5 text-sm font-bold text-white transition-all hover:bg-slate-800 hover:scale-105">
        ...
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 text-center flex flex-col items-center">
          ...
          >
            ...
            <div className="mt-12 flex flex-col sm:flex-row items-center justify-center gap-4 w-full">
              <Link to={currentUser ? '/dashboard' : '/login'} className="w-full sm:w-auto flex justify-center items-center gap-2 rounded-full bg-emerald-600 px-8 py-4 text-lg font-bold text-white transition-all hover:bg-emerald-700 hover:scale-105 shadow-md hover:shadow-xl">
                {currentUser ? 'Go to Dashboard' : 'Get Started Now'} <ArrowRight size={20} />
              </Link>
              <Link to={currentUser ? '/search' : '/login'} className="w-full sm:w-auto flex items-center justify-center gap-2 rounded-full border border-slate-200 bg-transparent hover:bg-white px-8 py-4 text-lg font-bold text-slate-600 transition-all hover:scale-105">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-8 md:grid-cols-3">
            {[
              ...
              <Link key={idx} to={currentUser ? (feature.link === '/demo/pharmacy' ? '/portal' : '/dashboard') : '/login'} className="group rounded-2xl bg-white p-8 shadow-sm transition-all hover:-translate-y-1 hover:shadow-md">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            >
              ...
              {currentUser ? 'Go back to Dashboard' : 'Sign in with Google to get started'}
            </Link>
          </motion.div>
        </div>
      </section>
